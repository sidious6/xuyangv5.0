...
export async function POST(req: Request) {
  try {
    ...
    console.error('Symptom log API error:', error);
...
export async function GET(req: Request) {
  try {
    ...
    // 获取症状记录
    const { data, error } = await db.getSymptomLogsByDate(user.id, date);
    ...
    console.error('Symptom log GET API error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
